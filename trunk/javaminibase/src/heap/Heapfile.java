  ...
  */
  private HFPage _newDatapage(DataPageInfo dpinfop)
	   ...
	   IOException
    {
      ...
      pageId = newPage(apage, 1);	//calls buffermgr and returns pageid for 'apage'
 ...
 * @throws HashOperationException 
  ...
  public RID insertRecord(byte[] recPtr) 
    throws InvalidSlotNumberException,  
	   ...
	   HFBufMgrException,
	   HFDiskMgrException,
	   IOException, HashOperationException, PageUnpinnedException, PagePinnedException, PageNotFoundException, BufMgrException
    {
      int dpinfoLen = 0;	
  ...
  public int getPositionForRID(RID rid) throws HFBufMgrException, IOException, InvalidSlotNumberException, InvalidTupleSizeException, HFException, HFDiskMgrException
  {
	  PageId currentDirPageId = new PageId(_firstDirPageId.pid);
      HFPage currentDirPage = new HFPage();
      PageId nextDirPageId = new PageId();
      ...
      pinPage(currentDirPageId, currentDirPage, false/*read disk*/);// I retreive the currentDirPage here
      Tuple atuple = new Tuple();
  	  ...
  	  for( currentDirPageRid = currentDirPage.firstRecord();
  	       ...
  	       currentDirPageRid = currentDirPage.nextRecord(currentDirPageRid))
  	    {
  		    ...
  		    pinPage(dpinfo.pageId, currentDataPage, false/*Rddisk*/);// I retreive the currentDataPage here
  		      ...
  		      if(dpinfo.pageId.pid==rid.pageNo.pid)//I found the required DataPage
  		      {
  			  //count the position by iterating thru all the records
  		    	  for( currentDataPageRid =  currentDataPage.firstRecord();
  		  	       currentDataPageRid.slotNo != rid.slotNo;
  		  	       currentDataPageRid = currentDataPage.nextRecord(currentDataPageRid))
  		    	  {
  		    		  position++;
  		    	  }
  		    	  unpinPage(dpinfo.pageId, false /*undirty*/);
  		
  		    	  return ++position;//I am done
  		      }
  		      else
  		      {  	
  		    	  // count the number of records in this datapage and add to the position
  		      position += currentDataPage.getSlotCnt();
  			  unpinPage(dpinfo.pageId, false /*undirty*/);// user record not found on this datapage; unpin it and try the next one
  		      }
  	    }
    ...
    return (Integer)null;
  }//end of getPositionForRID(Rid)
  
  public RID getRIDFromPosition(int position) throws IOException, HFBufMgrException, InvalidSlotNumberException, InvalidTupleSizeException
  {
	  PageId currentDirPageId = new PageId(_firstDirPageId.pid);
      HFPage currentDirPage = new HFPage();
      PageId nextDirPageId = new PageId();
	  	  ...
	  	  for( currentDirPageRid = currentDirPage.firstRecord();
	  	       ...
	  	       currentDirPageRid = currentDirPage.nextRecord(currentDirPageRid))
	  	    {
	  		    ...
	  		    pinPage(dpinfo.pageId, currentDataPage, false/*Rddisk*/);// I retreive the currentDataPage here
	  		      ...
	  		      if(position > currentDataPage.getSlotCnt())
	  		      {  			  
	  		    	  position -= currentDataPage.getSlotCnt();
  ...
  public ArrayList<RID> getallRID() throws IOException, HFBufMgrException, InvalidSlotNumberException, InvalidTupleSizeException
  {
	  ArrayList<RID> allRIDs = new ArrayList<RID>();
	  int counter = 0;
	  	  ...
	  	  for( currentDirPageRid = currentDirPage.firstRecord();
	  	       ...
	  	       currentDirPageRid = currentDirPage.nextRecord(currentDirPageRid))
	  	    {
	  		    ...
	  		    for( currentDataPageRid =  currentDataPage.firstRecord();
			  	       ...
			  	       currentDataPageRid = currentDataPage.nextRecord(currentDataPageRid))
			    	  {
			    		 allRIDs.add(currentDataPageRid);
			    		 counter++;
			    	  }
	  		  unpinPage(dpinfo.pageId, false /*undirty*/);	 
	  	    }
