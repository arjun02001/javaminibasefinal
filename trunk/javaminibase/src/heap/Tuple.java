...
//       fldOffset = fromTuple.copyFldOffset(); 
   ...
   public void modifyTuple(Tuple fromTuple) throws IOException{
	   data=fromTuple.getTupleByteArray();
	   tuple_length=fromTuple.getLength();
	   tuple_offset=0;
	   fldCnt=Convert.getShortValue(0, data);
	   fromTuple.fldCnt=fldCnt;
	   fldOffset=new short[fldCnt+1];
	   fromTuple.fldOffset=fldOffset;
	   int pos=2;
	   for(int i=0;i<=fldCnt;i++){
		   short x=Convert.getShortValue(pos, data);
		   fldOffset[i]=x;
		   fromTuple.fldOffset[i]=x;
		   pos+=2;
	   }
   }
